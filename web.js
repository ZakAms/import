...
var saveImages = function (v, cb) {
  ...
  var dir = path.join(path.resolve('../../../client/assets/public'), pathname.replace(f, ''));
...
var isRootScan = function (item) {
  return (item.url === "http://www.zappos.com/null-page1/.zso?p=0&s=goliveRecentSalesStyle/desc/");
}
...
var prepareNodesForUpdate = function (roots) {
  ...
  roots.forEach(function (root) {
    ...
    Node.create(root, function (err) {
      if (err)
        return console.error("can not create root._id = %s", root._id);
    ...
    products.forEach(function (product) {
      ...
      tasks.push(function (cb1) {
        ...
        driver.get(product.url)
          /*.then(function () { // price
            return driver.findElement(By.xpath("//*[@id='priceSlot']/span"));
          })
          .then(function (e) {
            return e.getText();
          }, function (err) {
            ignore = true;
            console.error("updateProductDetails: can not extract price for product " + product._id);
            //cb1(err);
          })
          .then(function (price) {
            if (!ignore)
